...
def send_block(data_field, last_block, counter):
    ...
    signature = private_key.sign(json.dumps(block).encode('utf-8'), padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=32), hashes.SHA256())
    block['signature'] = binascii.b2a_hex(signature).decode()
    block['hash'] = hashlib.sha256(json.dumps(block).encode('utf-8')).hexdigest()

    print("hash", hash0, block['hash'])
    print("signature", signature0, block['signature'])
